...
const Forgot = () => {
  ...
  const verifyHandler = async (e) => {
    ...
    try {
      ...
      if (data.status === "EMAIL_ALREADY_VERIFIED" || data.status === "VERIFICATION_SUCCESSFULL") {
  ...
  return (
    <div className="m-container">
      <div className='forgot-container'>
        ...
        {
          isOtpSended ? <input type="text" placeholder="Enter OTP Received On Email Address" className="otp-input" value={otp} onChange={(e) => {
            setOtp(e.target.value)
          }} required /> : null
        }

        <div className='btn-container'>
          <button className='reset-btn' onClick={isOtpSended ? verifyHandler : submitHandler}>{isOtpSended ? `Verify OTP` : `SEND OTP`}</button>
          <button className='login-btn' onClick={loginHandler}>Back To Login</button>
        </div>

        <span onClick={resendHandler} id="resend-otp">Resend Otp</span>
      </div>
    </div>
  )
}
